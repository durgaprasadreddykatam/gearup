...
const Locationprop = (props) => {
    return (
        <div onClick={() => props.handleClick(props.item.id)} className='relative flex-shrink-0 overflow-x-auto'>
            {props.item.image ? (<Image src={props.item.image.src} width={200} height={200} alt="" className="w-80 h-60 rounded-xl cursor-pointer m-3" />) :(<div className="w-80 h-60 rounded-xl cursor-pointer m-3 bg-gray-300"></div>)}
